...
size_t unique_words(char (*src1)[WORD_SIZE], size_t len1, char (*src2)[WORD_SIZE], size_t len2, char (*dst)[WORD_SIZE])
{
    ...
    for (size_t i = 0; i < len1; ++i)
        if (!contains(src2, len2, src1[i]) && !contains(src1, i, src1[i]) && !contains(src1 + i + 1, len1 - i - 1, src1[i]))
        {
    ...
    for (size_t i = 0; i < len2; ++i)
        if (!contains(src1, len1, src2[i]) && !contains(src2, i, src2[i]) && !contains(src2 + i + 1, len2 - i - 1, src2[i]))
        {
